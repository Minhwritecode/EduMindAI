...
class _NotebookDetailPageState extends State<NotebookDetailPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadChatHistory() async {
    ...
    if (history != null && err == null) {
      setState(() {
        messages = history.map((msg) => ChatMessage(
          user: msg.sender == 'user' ? currentUser : geminiUser,
          createdAt: msg.timestamp != null ? DateTime.parse(msg.timestamp!) : DateTime.now(),
          text: msg.messageText,
        )).toList().reversed.toList(); // DashChat expects newest first
      });
    }
    ...
    if (mounted) {
      setState(() {
        ...
        if (err == null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Đã lưu nguồn thành công!')));
        } else {
          ...
          builder: (context, setDialogState) {
            return DefaultTabController(
              ...
              child: AlertDialog(
                ...
                title: Text('Thêm Nguồn Học Tập', style: TextStyle(color: _textColor)),
                content: SizedBox(
                  width: 450,
                  height: 360,
                  child: Column(
                    children: [
                      TabBar(
                        ...
                        tabs: const [
                          ...
                          Tab(icon: Icon(Icons.upload_file), text: 'Tải lên tệp'),
                        ],
                      ),
                                    ...
                                    if (pickedFileName == null) ...[
                                      InkWell(
                                        ...
                                        child: Container(
                                          ...
                                          child: Column(
                                            children: [
                                              Icon(Icons.cloud_upload_outlined, size: 48, color: _accentColor),
                                              const SizedBox(height: 12),
                                              Text(
                                          ...
                                          children: [
                                            Icon(
                                              pickedFileName!.endsWith('.pdf') 
                                                  ...
                                                  : pickedFileName!.endsWith('.docx')
                                                      ? Icons.description
                                                      : pickedFileName!.endsWith('.pptx')
                                                          ? Icons.slideshow
                                                          : Icons.article,
                                              color: _accentColor,
                                            ),
                ...
                actions: [
                  ...
                  Builder(
                    builder: (btnCtx) => FilledButton(
                      ...
                      onPressed: isUploadingFile ? null : () async {
                        final tabIndex = DefaultTabController.of(btnCtx).index;
                        final userId = context.read<NotebookContextState>().userId;
                        ...
                        if (_notebook.id.isEmpty) {
                          ...
                          final (newId, err) = await NotebookMongoSync.saveNotebook(
                            userId: userId,
                            id: '',
                            title: _notebook.title,
                            description: _notebook.description,
                          ...
                          if (err != null || newId == null || newId.isEmpty) {
                            setDialogState(() {
                              isUploadingFile = false;
                            });
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('Lỗi khởi tạo notebook: $err')),
                            );
                            return;
                          }
                          setState(() {
                        ...
                        if (tabIndex == 0) {
                          // Manual text add
                          final title = _sourceTitleController.text.trim();
                          final content = _sourceContentController.text.trim();
                          if (title.isEmpty || content.isEmpty) return;
                          Navigator.pop(context);

                          setState(() => _isSaving = true);
                          final (sourceId, err) = await NotebookMongoSync.uploadSource(
                          ...
                          _handleSourceUploadResult(sourceId, err, userId);
                        } else {
                          // File pick upload
                          if (pickedFileBytes == null || pickedFileName == null) return;
                          
                          setDialogState(() {
                            isUploadingFile = true;
                          });

                          final (sourceId, err) = await NotebookMongoSync.uploadSourceFile(
                            notebookId: _notebook.id,
                            userId: userId,
  ...
  void _handleSourceUploadResult(String? sourceId, String? err, String userId) async {
    setState(() => _isSaving = true);
    if (err == null && sourceId != null) {
      ...
      if (updatedNbs != null) {
        ...
        setState(() {
          _notebook = found;
          _selectedSource = _notebook.sources.firstWhere((s) => s.sourceId == sourceId);
          _sourceViewerController.text = _selectedSource!.content;
        });
      }
      _sourceTitleController.clear();
      _sourceContentController.clear();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Đã tải lên và trích xuất nguồn thành công!')));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Lỗi upload: $err')));
    }
  ...
  Future<void> _sendMessage(ChatMessage chatMessage) async {
    ...
    if (err == null && aiResponse != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('AI Error: $err')));
    }
  }
  ...
  Future<void> _generateAsset(String type) async {
    ...
    if (type == 'mindmap') {
      final (mindmap, err) = await NotebookMongoSync.generateMindmap(_notebook.id, userId, 'Bản Đồ Tư Duy');
      if (err == null && mindmap != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Lỗi Mindmap: $err')));
      }
    } else if (type == 'quiz') {
      final (quiz, err) = await NotebookMongoSync.generateQuiz(_notebook.id, userId, 'Bài Kiểm Tra');
      if (err == null && quiz != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Lỗi Quiz: $err')));
      }
    } else if (type == 'flashcards') {
      final (deck, err) = await NotebookMongoSync.generateFlashcards(_notebook.id, userId, 'Thẻ Ghi Nhớ');
      if (err == null && deck != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Lỗi Flashcards: $err')));
      }
    }
    setState(() => _isSaving = false);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Theme(
      data: ThemeData.dark().copyWith(
        ...
        colorScheme: ColorScheme.dark(primary: _accentColor, surface: _panelColor),
      ),
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text(_notebook.title, style: TextStyle(color: _textColor, fontSize: 18, fontWeight: FontWeight.bold)),
          centerTitle: false,
          bottom: TabBar(
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                Text('Nguồn Học Tập', style: TextStyle(color: _textColor, fontSize: 16, fontWeight: FontWeight.bold)),
                const Spacer(),
          ...
          Padding(
            ...
            child: OutlinedButton.icon(
              ...
              label: const Text('Thêm nguồn mới', style: TextStyle(color: Colors.white)),
              ...
              ? Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Chưa có tài liệu nguồn nào.', style: TextStyle(color: _textColor.withOpacity(0.5))),
                    ],
                  ),
                )
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final isSelected = _selectedSource?.sourceId == source.sourceId;
                    return ListTile(
                      selected: isSelected,
                      selectedColor: _accentColor,
                      leading: Icon(Icons.description, color: isSelected ? _accentColor : Colors.white60),
                      title: Text(source.title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                      subtitle: Text('${source.content.length} ký tự', style: const TextStyle(fontSize: 11, color: Colors.white38)),
                      onTap: () {
                        setState(() {
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Text(_selectedSource!.title, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white60)),
                      const Spacer(),
                      const Text('Trình soạn thảo', style: TextStyle(fontSize: 10, color: Colors.white38)),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Expanded(
                    child: Container(
                      decoration: BoxDecoration(color: _panelColor, borderRadius: BorderRadius.circular(8)),
                      child: TextField(
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                Text('Studio Sáng Tạo', style: TextStyle(color: _textColor, fontSize: 16, fontWeight: FontWeight.bold)),
                const Spacer(),
          ...
          if (_notebook.mindmaps.isNotEmpty || _notebook.quizzes.isNotEmpty || _notebook.flashcards.isNotEmpty) ...[
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8),
              child: Text('TÀI SẢN ĐÃ TẠO', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: _accentColor)),
            ),
            Expanded(
              child: ListView(
                ...
                children: [
                  ..._notebook.mindmaps.map((m) => ListTile(
                    leading: const Icon(Icons.schema, size: 18, color: Colors.amber),
                  ...
                  ..._notebook.quizzes.map((q) => ListTile(
                    leading: const Icon(Icons.quiz, size: 18, color: Colors.green),
                    title: Text(q.title, style: const TextStyle(fontSize: 12)),
                    onTap: () => setState(() {
                      _selectedQuiz = q;
                      _currentQuizQuestionIndex = 0;
                      _selectedQuizOption = null;
                  ...
                  ..._notebook.flashcards.map((f) => ListTile(
                    leading: const Icon(Icons.style, size: 18, color: Colors.purple),
  ...
  Widget _buildMindmapTab() {
    final item = _selectedMindmap ?? (_notebook.mindmaps.isNotEmpty ? _notebook.mindmaps.last : null);
    if (item == null) {
      return _buildStudioPlaceholder('Sơ Đồ Tư Duy', 'Nhấp vào nút "Tạo Sơ Đồ Tư Duy" ở bảng bên phải để bắt đầu.');
    }
              ...
              child: Container(
                ...
                child: Stack(
                  children: [
                    ...
                    ...edges.map((edge) {
                      final srcNode = nodes.firstWhere((n) => n.id == edge.source, orElse: () => nodes[0]);
                      final destNode = nodes.firstWhere((n) => n.id == edge.target, orElse: () => nodes[0]);
                      return CustomPaint(
                    ...
                    ...nodes.map((node) {
                      ...
                      return Positioned(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            gradient: isRoot 
                              ? LinearGradient(colors: [_accentColor, const Color(0xFF2C6B69)])
                              : const LinearGradient(colors: [Color(0xFF2D2E32), Color(0xFF232427)]),
  ...
  Widget _buildQuizTab() {
    final quiz = _selectedQuiz ?? (_notebook.quizzes.isNotEmpty ? _notebook.quizzes.last : null);
    if (quiz == null) {
      return _buildStudioPlaceholder('Bộ Trắc Nghiệm', 'Hãy bấm nút sinh trắc nghiệm ở studio bên phải để AI trích xuất câu hỏi ôn tập.');
    }
          ...
          Row(
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(color: _accentColor.withOpacity(0.15), borderRadius: BorderRadius.circular(20)),
                child: Text('Đúng: $_quizScore/${questions.length}', style: TextStyle(color: _accentColor, fontWeight: FontWeight.bold, fontSize: 13)),
              )
            ],
          ),
          ...
          if (!_quizAnswered)
            FilledButton(
              ...
              onPressed: _selectedQuizOption == null ? null : () {
                setState(() {
                  _quizAnswered = true;
                  if (_selectedQuizOption == currentQuestion.correctAnswer) {
                    _quizScore++;
                  }
                });
              },
              child: const Text('Kiểm Tra Đáp Án', style: TextStyle(fontWeight: FontWeight.bold)),
            )
          else
            FilledButton(
              style: FilledButton.styleFrom(
              ...
              child: Text(
                _currentQuizQuestionIndex < questions.length - 1 ? 'Câu Tiếp Theo' : 'Chơi Lại',
  ...
  Widget _buildFlashcardsTab() {
    final deck = _selectedFlashcardDeck ?? (_notebook.flashcards.isNotEmpty ? _notebook.flashcards.last : null);
    if (deck == null) {
      return _buildStudioPlaceholder('Thẻ Ghi Nhớ', 'Bấm vào nốt tạo thẻ ghi nhớ ở studio bên phải để AI tóm tắt các định nghĩa học nhanh.');
    }
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(deck.title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              Text('${_currentFlashcardIndex + 1}/${cards.length}', style: const TextStyle(color: Colors.white54)),
            ],
          ),
          const SizedBox(height: 32),
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  gradient: _flashcardFlipped 
                    ? const LinearGradient(colors: [Color(0xFF2C6B69), Color(0xFF1E3F3E)])
                    : const LinearGradient(colors: [Color(0xFF2D2E32), Color(0xFF1E1F22)]),
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      _flashcardFlipped ? 'ĐÁP ÁN / ĐỊNH NGHĨA' : 'THUẬT NGỮ / CÂU HỎI',
                      style: TextStyle(color: _accentColor, fontWeight: FontWeight.bold, fontSize: 12, letterSpacing: 1.5),
                    ),
                    const SizedBox(height: 24),
                    Text(
                    ...
                    const SizedBox(height: 8),
                    const Text('Chạm để lật thẻ', style: TextStyle(color: Colors.white24, fontSize: 11)),
                  ],
                ),
              ),
            ),
          ...
          Row(
            ...
            children: [
              IconButton.filledTonal(
                ...
                onPressed: _currentFlashcardIndex == 0 ? null : () {
                  setState(() {
              ...
              const Text('Chạm thẻ để xem đáp án', style: TextStyle(color: Colors.white38, fontSize: 12)),
              IconButton.filledTonal(
                icon: const Icon(Icons.arrow_forward_ios),
                onPressed: _currentFlashcardIndex == cards.length - 1 ? null : () {
                  setState(() {
                    _currentFlashcardIndex++;
                    _flashcardFlipped = false;
                  });
  ...
  Widget _buildStudioPlaceholder(String assetName, String instruction) {
    return Container(
      ...
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.auto_awesome, size: 64, color: _accentColor.withOpacity(0.4)),
          const SizedBox(height: 24),
          Text('Chưa có $assetName nào', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          Text(instruction, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white38, fontSize: 13, height: 1.4)),
        ],
      ),
    );
  }
